local mark = require("harpoon.mark")
local ui = require("harpoon.ui")

vim.keymap.set("n", "<leader>h", ui.toggle_quick_menu, {noremap= true})
vim.keymap.set("n", "<leader>ha", mark.add_file, {noremap = true})
vim.keymap.set("n", "<leader>1", function() ui.nav_file(1) end, {noremap= true})
vim.keymap.set("n", "<leader>2", function() ui.nav_file(2) end, {noremap= true})
vim.keymap.set("n", "<leader>3", function() ui.nav_file(3) end, {noremap= true})
vim.keymap.set("n", "<leader>4", function() ui.nav_file(4) end, {noremap= true})
vim.keymap.set("n", "<leader>5", function() ui.nav_file(5) end, {noremap= true})
vim.keymap.set("n", "<leader>6", function() ui.nav_file(6) end, {noremap = true})
vim.keymap.set("n", "<leader>7", function() ui.nav_file(7) end, {noremap = true})
vim.keymap.set("n", "<leader>8", function() ui.nav_file(8) end, {noremap = true})
vim.keymap.set("n", "<leader>9", function() ui.nav_file(9) end, {noremap = true})
vim.keymap.set("n", "<leader>0", function() ui.nav_file(10) end, {noremap = true})

